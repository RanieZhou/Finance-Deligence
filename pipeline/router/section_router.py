...
_CHAPTER_RULES: list[tuple[re.Pattern, str]] = [
    ...
    # 股权与控制关系
    (re.compile(r"股权结构|控制关系|实际控制人|控股股东|持有.*5%以上|持有.*股份的股东|股东及实际控制人|股权关系|控股关系|主要股东|股权与控制|前十大股东|股东情况|持股情况"), TARGET_OWNERSHIP),
    # 财务指标
    ...
    # 风险事项
    (re.compile(r"风险因素|重大风险|风险提示|重大事项提示|风险因素及对策"), TARGET_RISK),
    # 合规事项
    (re.compile(
        r"处罚|诉讼|仲裁|关联交易|对外担保|承诺事项|或有事项|其他重要事项|法律事项|重要合同|"
        r"重大合同|或有负债|法律法规|重大事项|独立性|违规|合规|保荐人核查|法律意见"
    ), TARGET_COMPLIANCE),
]
